...
## BUILT 2026-09-24 — Phase 2, staged ranking
Tim: "begin phase 2". `runGoalRank` scores offers in the finder's order; once `GOAL_STAGE = 10` are in, `stageGoalRank` sorts those ten by `compareByGoal`, marks their ties, and repaints them at the top. Every row under them gets `goalUnranked`: `tr.unranked` (faded to 0.55 in trade.html), no rank number, and no % even after it has been scored, because it is still in points order. The status line reads "top 10 ranked by your title chance; faded rows in points order; playing each offer out (n of m)…", and "in points order; playing each offer out…" before the ten are in. It stages once, not every slice, so the table doesn't reshuffle while you read it. Stale scores from a half-finished earlier pass are cleared at the start of `runGoalRank` (only when the search is not yet goal-ranked).
**Verified how:** new tr-test scenario `goalStaged` (title goal, demo) polls every 20 ms and keeps the first snapshot with ranked rows while the line still says it is working: 10 ranked rows on top in value order, 30 marked unranked with no %, line wording, and a clean finish. Run against HEAD's trade-page.js it caught no staged snapshot at all, so the key assertion fails there. Phone view at 393px: finished table and line fit, no overflow. The in-between moment lasts about a second in a real browser, so it was not caught on screen.

## BUILT 2026-09-24 — Phase 3, engine part 1
Tim: "start phase 3". Four parts:
- **Search once.** `loadWeekly` now buys every readable week (span + played weeks) before the single weekly `runSearch`, then `loadHistory`. Before, the span was bought, a points search ran, the played weeks landed, and `goalFollowUp` searched again with weights. It only shows with slow reads — the instant stub hid it — so `tests/tr-stub-season.mjs` takes `TR_WEEK_DELAY` (ms per week read). With 150 ms: old page 2 weekly searches on both goals, new page 1 (counter: `data-weekly-searches` on `#tradeTable`).
- **Base sim reused.** `weekWeights(..., { base })` takes the page's own base run (`goalWeightsFor` passes `ctx.base`), instead of simulating it a second time.
- **One ESPN-look.** `espnLookPerWeek(offer, n, totalOf)` in trade-odds.js is the only definition; the page's `acceptFor` calls it with its span totals.
- **His side over the weeks he plays.** `playoffReach(base, inputs, teamId, weeks)`: regular week 1; round one P(playoffs) − P(bye); later round r P(top size÷2^r places). This refines the plan's plain `pPlayoffs` weighting — a bye is a week off, and the final is reached less often than the playoffs. `findTrades({ theirReach })` uses it for His gain, his loss tolerance (−2 × Σreach) and the candidate ceiling; offers carry `theirPoints` (flat) and `theirWeeks` (Σreach). The page's He gains column, the yes-curve (`acceptFor` divides by `theirWeeks`), scoreGoal, the combo and the custom box all go through `hisSideOf`. The note now opens its bracket paragraph with "He gains counts each playoff week by the chance he plays in it…" (wording is Tim's to change).
**Verified how:** tr-test `searchOnce` ×2 goals (fails on the old page: 2 searches); test-trade-odds §17 (playoffReach against a hand fixture, the base reused, espnLookPerWeek with the page's totals, findTrades His gain = Σ reach × his weekly change by hand) — 8 of those fail on the old engine even with `playoffReach` added to it; tr-test pins the note sentence. Full suite (`run-all --bless`, 755 s): 42/43, the one failure being two tr-test assertions that pin read ORDER (the span first, the played weeks behind it); the single batch now keeps that order and tr-test is 670/670 (up from 660), test-trade-odds 128 (up from 111). Phone view at 393px: no overflow, finder ranking.
