...
### The five next steps — all built the same day (Tim: "go ahead and deploy 1-5 now")

1. **The goal chooses the candidates.** Each priced week is weighted by how
   far ten extra points in it move the chance, and the finder keeps deals by
   the weighted gain. Measured on demo as of week 5, title goal: the best
   expected title gain on offer went from **+2.4% to +6.8%**, and 7 of the 40
   finalists LOSE points — invisible to the old finder. The weights read like
   the season does: October ×0.2–0.6, weeks 14–16 ×1.6–3.6. Weights cost
   under a second; the search stayed at ~4.5s.
2. **2-for-2**, bounded to each side's top ten pieces: 15 of the 40 demo
   finalists are 2-for-2s.
3. **Best combo, custom trades and the pop-up** all read one shared
   simulation context, so four panels cannot quote four baselines.
4. **The emptied roster spot turned out to be credited already** whenever the
   wire is read — the floor lifts exactly the slots the short roster cannot
   fill. The finding is the correction: the method note had been telling
   Tim the opposite. Proved in `test-trade-odds.mjs` §13 rather than argued.
5. **The trade deadline** from `settings.tradeSettings`, under the title.

Falsified: the combo goal line removed (1 fail), the deadline never shown
(2), the finder ignoring the weights (3).

### Next

- **The yes-chance is still one judgement nobody has checked against a real
  accepted or refused trade.** If Tim proposes deals from this page, the
  outcome of each is the only evidence there will ever be for the curve.
- **Suggested players** in the custom box (`js/trade-suggest.js`) still rank
  by points, not by the goal.
- **Three-for-anything** is still unsearched; ESPN allows it.
