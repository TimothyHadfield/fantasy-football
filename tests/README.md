...
**12,089 assertions across 37 suites**, counted off a green run on 2026-09-19
(about 10 minutes depending on what else the machine is doing — `tr-test` alone
is 3½ of them since the Trade page started reading itself on load). Four of the
37 report pages or scenarios rather than a count, so they are not in that
