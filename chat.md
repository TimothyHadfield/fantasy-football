...
- **Open:** branch `tr-test` 601/603 (two fixture-dependent assertions, see PROGRESS Status); then merge + push + tell Tim to hard-refresh. Found by tests on the branch: combo emptied without partner tolerance (fixed); finder picked only fleeces without EV keep-order (fixed).
